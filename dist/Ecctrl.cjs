...
function EcctrlAnimation(props) {
  ...
  const { scene } = drei.useGLTF(props.characterURL);
  const [animationsAdded, setAnimationsAdded] = React.useState(false);
  React.useEffect(() => {
  ...
  const { actions, mixer } = drei.useAnimations(
  ...
  React.useEffect(() => {
    if (actions && animationsAdded && group.current) {
      const action = actions[curAnimation] || actions[props.animationSet.jumpIdle];
